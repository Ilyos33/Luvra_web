...
if (!connectionString) {
  console.error('КРИТИЧЕСКАЯ ОШИБКА: Переменная DATABASE_URL не найдена!\nПроцесс продолжит работу с заглушкой, но все запросы к БД будут возвращать понятную ошибку.');

  module.exports = {
    query: async () => {
      throw new Error('DATABASE_URL не настроен в окружении — подключение к базе данных недоступно. Установите переменную окружения DATABASE_URL и перезапустите приложение.');
    },
    // initDb — noop заглушка
    initDb: async () => {},
  };
